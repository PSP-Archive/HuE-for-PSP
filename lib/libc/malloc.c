...
// こっからmallocなどの実装というか、らっぱー(hey yo!)

extern char malloc_mem[]; // 外部で定義すること
extern int  malloc_size;  // 外部で定義すること
//char _buf_mem_[MALLOC_MEMSIZE];

static int alloc_sema = -1;

static mspace psp_msp=0;

static void alloc_lock(void)
{
    if(alloc_sema>=0) {
        sceKernelWaitSema(alloc_sema,1,0);
    }
}
...
{
    if(alloc_sema>=0) {
        sceKernelSignalSema(alloc_sema,1);
    }
}

int init_malloc(int ispsp)
{
    psp_msp = create_mspace_with_base((void*)malloc_mem,malloc_size,0);
    ...
    alloc_sema = sceKernelCreateSema("AllocMutex", 0, 1, 1, NULL);
...
void* realloc(void* mem, size_t newsize)
{
    ...
    alloc_lock();
    p = mspace_realloc(psp_msp,mem,newsize);
    alloc_unlock();
...
void * memalign(size_t align, size_t size)
{
    void* p;
    alloc_lock();
    p = mspace_memalign(psp_msp,align,size);
    alloc_unlock();
    return p;
}
